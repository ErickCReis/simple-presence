...
Under the hood, the core SDK deduplicates connections. Multiple `SimplePresence` instances with the same `(apiUrl, appKey, tag)` tuple share a single WebSocket. The connection is only closed when _all_ instances using it have been destroyed.
