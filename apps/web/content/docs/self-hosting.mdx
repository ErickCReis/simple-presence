...
<Callout type="info" title="Before you begin">
  Self-hosting requires a [Cloudflare Workers Paid
  plan](https://www.cloudflare.com/plans/developer-platform/) ($5/mo) for Durable Objects support.
  You'll also need [Bun](https://bun.sh) installed locally.
</Callout>

## Architecture

<Cards>
  <Card title="Workers">HTTP API, auth, WebSocket upgrade routing</Card>
  <Card title="Durable Objects">Real-time state, WebSocket management, count aggregation</Card>
  <Card title="D1 Database">User accounts, app registry, persistent data</Card>
</Cards>
...
<Callout title="Prefer the hosted version?">
  The free tier includes 2 apps and 100 concurrent connections — enough for most projects. No
  infrastructure to manage. [Get started for free →](/dashboard)
</Callout>
